...
if(!MONGO_URI) console.log("❌ NO MONGO_URI");
...
app.get('/api/health',(req,res)=>res.json({ok:true,mongo:mongoose.connection.readyState, v:"705 PRO"}));
...
app.post('/api/auth/login',async(req,res)=>{
 try{let {email,password}=req.body;let u=await User.findOne({email:(email||'').toLowerCase()}); if(!u) return res.status(400).json({error:"User not found"});
...
app.get('/api/me',protect,async(req,res)=>{
 try{let u=await User.findById(req.user.id).select("email role name upiId"); res.json(u);}catch(e){res.status(500).json({error:e.message})}
});

app.post('/api/save-upi',protect,async(req,res)=>{
 try{
  let {upiId}=req.body; if(!upiId||!upiId.includes("@")) return res.status(400).json({error:"Valid UPI like name@upi required"});
  upiId=upiId.trim(); let u=await User.findByIdAndUpdate(req.user.id,{upiId},{new:true});
  await Novel.updateMany({creatorEmail:u.email},{authorUpi:upiId,upiId,upi:upiId});
  await Comic.updateMany({creatorEmail:u.email},{authorUpi:upiId,upiId,upi:upiId});
...
app.post('/api/fix-upi-all',protect,async(req,res)=>{
 try{
  let u=await User.findById(req.user.id); if(!u.upiId) return res.status(400).json({error:"Set UPI first in Dashboard"});
  let n=await Novel.updateMany({creatorEmail:u.email},{authorUpi:u.upiId,upiId:u.upiId,upi:u.upiId});
  let c=await Comic.updateMany({creatorEmail:u.email},{authorUpi:u.upiId,upiId:u.upiId,upi:u.upiId});
  res.json({ok:true,count:(n.modifiedCount||0)+(c.modifiedCount||0),upiId:u.upiId});
 }catch(e){res.status(500).json({error:e.message})}
});
...
app.post('/api/novels',protect,async(req,res)=>{
 try{
  let d=req.body; if(!d.title) return res.status(400).json({error:"Title required"});
  let u=await User.findById(req.user.id); let doc=await Novel.create({...d,creatorEmail:u.email,creatorName:u.name||u.email,authorUpi:d.authorUpi||d.upiId||u.upiId||"",upiId:d.authorUpi||u.upiId||"",slug:slugify(d.title),views:0});
  res.json(doc);
 }catch(e){res.status(500).json({error:e.message})}
});
...
app.post('/api/comics',protect,async(req,res)=>{
 try{
  let d=req.body; if(!d.title) return res.status(400).json({error:"Title required"});
  let pages=cleanPages(d.pages); if(pages.length===0 &&!d.cover) return res.status(400).json({error:"At least 1 valid image URL required"});
  let u=await User.findById(req.user.id); let doc=await Comic.create({...d,pages,creatorEmail:u.email,creatorName:u.name||u.email,authorUpi:d.authorUpi||d.upiId||u.upiId||"",upiId:d.authorUpi||u.upiId||"",pageCount:pages.length,slug:slugify(d.title),views:0});
  res.json(doc);
 }catch(e){res.status(500).json({error:e.message})}
});
...
app.listen(PORT,()=>console.log(`✅ Gen-Z Visual v705 PRO FINAL CLOSED running on ${PORT}`));
