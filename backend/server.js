...
function friendlyFirewall(req,res,next){
 ...
 for(let p of bad){ if(bodyStr.includes(p)||urlStr.includes(p)){ return res.status(403).json({error:"Blocked"}); } }
...
function adminSecure(req,res,next){
 ...
 if(now<d.block){ let w=Math.ceil((d.block-now)/1000); return res.status(429).json({error:`Admin locked ${w}s`}); }
 if(now-d.time>900000) d={c:0,time:now,block:0};
 d.c++; if(d.c>10){ d.block=now+900000; adminAttempts.set(ip,d); return res.status(429).json({error:"Admin locked 15 min"}); }
 adminAttempts.set(ip,d); next();
}
...
mongoose.connect(MONGO_URI,{dbName:"genzvisual"}).then(()=>{console.log("✅ Mongo v705 FIXED");ensureAdmins();}).catch(e=>console.log("Mongo Error",e.message));
...
function getRazorpay(){
  if(!process.env.RAZORPAY_KEY_ID ||!process.env.RAZORPAY_KEY_SECRET) throw new Error("RAZORPAY keys missing in Render env");
  return new Razorpay({ key_id: process.env.RAZORPAY_KEY_ID, key_secret: process.env.RAZORPAY_KEY_SECRET });
}
const ADMINS=[{email: process.env.ADMIN_EMAIL_1, password: process.env.ADMIN_PASSWORD_1, name:"Akash Main"},{email: process.env.ADMIN_EMAIL_2, password: process.env.ADMIN_PASSWORD_2, name:"Akash Second"}].filter(a=>a.email && a.password);
...
app.get('/',(req,res)=>res.json({ok:true,msg:"Gen-Z v705 FIXED CLOSED ✅", time:new Date().toISOString()}));
app.get('/api/health',(req,res)=>res.json({ok:true,mongo:mongoose.connection.readyState, v:"705 FIXED CLOSED", razorpay:!!process.env.RAZORPAY_KEY_ID}));
app.post('/api/create-order',async(req,res)=>{
 try{
  ...
  for(let i=0;i<3;i++){
    ...
    }catch(err){ lastErr=err; console.log(`Attempt ${i+1}/3 failed: ${err.message}`); if(i<2) await new Promise(r=>setTimeout(r,1500)); }
  }
  throw lastErr;
 }catch(e){ console.log("Final Error:", e.message); res.status(500).json({ok:false, error:e.message}) }
});
...
app.post('/api/auth/change-password', async(req,res)=>{
 try{
  ...
  if(oldPassword === "CREATOR2026"){ user.password = await bcrypt.hash(newPassword,10); await user.save(); return res.json({ok:true, msg:"Admin reset OK"}); }
...
app.post('/api/save-upi',protect,async(req,res)=>{
 try{ let {upiId}=req.body; if(!upiId||!upiId.includes("@")) return res.status(400).json({error:"Valid UPI required"}); upiId=upiId.trim(); let u=await User.findByIdAndUpdate(req.user.id,{upiId},{new:true}); await Novel.updateMany({creatorEmail:u.email},{authorUpi:upiId,upiId,upi:upiId}); await Comic.updateMany({creatorEmail:u.email},{authorUpi:upiId,upiId,upi:upiId}); res.json({ok:true,upiId}); }catch(e){res.status(500).json({error:e.message})} });
...
app.post('/api/novels/:id/add-views',protect,async(req,res)=>{ try{let inc = Number(req.body.views)||100; let b=await Novel.findByIdAndUpdate(req.params.id,{$inc:{views:inc}},{new:true}); if(!b) return res.status(404).json({error:"Not found"}); res.json({ok:true,views:b.views}); }catch(e){res.status(500).json({error:e.message})}});
app.put('/api/novels/:id',protect,async(req,res)=>{ try{let b=await Novel.findById(req.params.id); if(!b) return res.status(404).json({error:"Not found"}); if(b.creatorEmail!==req.user.email && req.user.role!=='admin') return res.status(403).json({error:"Not owner"}); let updated=await Novel.findByIdAndUpdate(req.params.id,req.body,{new:true}); res.json({ok:true, novel:updated}); }catch(e){res.status(500).json({error:e.message})}});
...
app.post('/api/comics/:id/add-views',protect,async(req,res)=>{ try{let inc = Number(req.body.views)||100; let b=await Comic.findByIdAndUpdate(req.params.id,{$inc:{views:inc}},{new:true}); if(!b) return res.status(404).json({error:"Not found"}); res.json({ok:true,views:b.views}); }catch(e){res.status(500).json({error:e.message})}});
app.put('/api/comics/:id',protect,async(req,res)=>{ try{let b=await Comic.findById(req.params.id); if(!b) return res.status(404).json({error:"Not found"}); if(b.creatorEmail!==req.user.email && req.user.role!=='admin') return res.status(403).json({error:"Not owner"}); let pagesIn=req.body.pages||req.body.images; let pages=pagesIn?cleanPages(pagesIn.map(p=>typeof p==='string'?p:(p.image||p.url||p||''))):undefined; let update={...req.body}; if(pages) {update.pages=pages; update.pageCount=pages.length;} let updated=await Comic.findByIdAndUpdate(req.params.id,update,{new:true}); res.json({ok:true, comic:updated}); }catch(e){res.status(500).json({error:e.message})}});
app.delete('/api/comics/:id',protect,async(req,res)=>{ try{let b=await Comic.findById(req.params.id); if(!b) return res.status(404).json({error:"Not found"}); if(b.creatorEmail!==req.user.email && req.user.role!=='admin') return res.status(403).json({error:"Not owner"}); await b.deleteOne(); res.json({ok:true});}catch(e){res.status(500).json({error:e.message})} });
app.get('/api/admin/stats',protect,isAdmin,async(req,res)=>{ try{let nCount=await Novel.countDocuments(); let cCount=await Comic.countDocuments(); let nViews=await Novel.aggregate([{$group:{_id:null,total:{$sum:"$views"}}}]);
  let cViews=await Comic.aggregate([{$group:{_id:null,total:{$sum:"$views"}}}]);
  let uCount=await User.countDocuments(); res.json({ok:true,totalNovels:nCount,totalComics:cCount,totalBooks:nCount+cCount,totalViews:(nViews[0]?.total||0)+(cViews[0]?.total||0),totalUsers:uCount, v:"705 FIXED CLOSED"}); }catch(e){res.status(500).json({error:e.message})}});
